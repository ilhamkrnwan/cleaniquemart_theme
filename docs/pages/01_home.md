# Dokumentasi Halaman: Beranda (Home)

> **URL Asli**: [https://cleaniquemart.com/](https://cleaniquemart.com/)  
> **Tipe Halaman**: Front Page / Master Conversion Landing Page  
> **Template WordPress Acuan**: `front-page.php`  
> **Fokus Konversi**: Pendaftaran Mitra Depot Sabun Curah & Konsultasi WhatsApp

---

## 1. Metadata & SEO Spec

| Parameter | Nilai / Konfigurasi |
| :--- | :--- |
| **Title Tag** | Peluang Usaha Bisnis 2026 Menjanjikan - Cleanique Mart |
| **Meta Description** | Jualan Sabun Curah Dengan Potensi Omzet 50+ Juta/Bulan!. Produk gampang laku Cocok untuk pemula, minim resiko, pasti balik modal |
| **Target Keyword** | Peluang usaha 2026, peluang usaha sabun curah, bisnis depo sabun, distributor kimia laundry |
| **Open Graph Title** | Peluang Usaha Bisnis 2026 Menjanjikan - Cleanique Mart |
| **Open Graph Desc** | Jualan Sabun Curah Dengan Potensi Omzet 50+ Juta/Bulan!. Produk gampang laku Cocok untuk pemula, minim resiko, pasti balik modal |
| **Canonical URL** | `https://cleaniquemart.com/` |
| **Schema Types** | `Organization`, `WebSite`, `LocalBusiness`, `FAQPage` |

---

## 2. Struktur Section & Wireframe Layout

1. **Top Bar**: Pengumuman Promo Kemitraan & Hotline WhatsApp `087885590088`.
2. **Navbar**: Logo Cleanique Mart, Menu Utama, Tombol "Konsultasi Gratis".
3. **Hero Section**: 
   - Badge: `"Peluang Usaha 2026 - Potensi Profit Besar"`.
   - H1: `"Peluang Usaha Aneka Sabun dengan Potensi Omzet 50+ Juta/Bulan!"`.
   - Tombol CTA Primer: `KONSULTASI GRATIS` (ke WhatsApp).
   - Tombol CTA Sekunder: `DOWNLOAD PROPOSAL` (Google Drive PDF).
4. **Agitasi & Solusi Masalah**:
   - Pain points pemula: Bingung mulai dari mana, tidak paham promosi, takut rugi/ditipu, terjebak impian finansial.
   - Solusi: Bisnis Aneka Sabun Auto Balik Modal - Inovasi sejak 2011, terjual > 1 Juta produk, pasar terjamin.
5. **Kriteria Calon Mitra**:
   - Punya toko/ruang kosong, bosan gaji pas-pasan, ingin bisnis modal terjangkau & repeat order tinggi.
6. **Market Segments (10 Pangsa Pasar Sabun Curah)**:
   - Laundry, Rumah Makan/Restoran, Rumah Tangga, Industri, Gedung/Kantor, Hotel/Penginapan, Salon, Klinik/RS, Cuci Kendaraan, Tempat Ibadah.
7. **Keunggulan Ekosistem Cleanique Mart (7 Pilar)**:
   - Repeat Order Tinggi, Terjual > 1 Juta Produk (Sejak 2011), Harga Kompetitif, Ramah Lingkungan, Pelatihan Produk & SOP, Izin Usaha/PKRT & Halal MUI, Dukungan Marketing AI & Offline.
8. **Katalog Varian Produk Curah (Jerigen 25 Liter)**:
   - Hand Wash, Deterjen Busa, Deterjen Matic, Parfum Waterbase, Softener, Pelicin Setrika, Sabun Cuci Piring, Pel Lantai.
9. **Social Proof & Testimoni Mitra Riil**:
   - Windu Darojat (Mitra Temanggung - 19 thn di household).
   - Johan Regan Jauw (Mitra Palembang).
   - Idha (Mitra Maguwoharjo Sleman).
10. **Kredibilitas Pabrik & Legalitas (PT Indotech Berkah Abadi)**:
    - Pengalaman 13+ tahun (sejak 2011), sertifikat produksi Kemenkes PKRT, hasil uji laboratorium.
11. **Review Marketplace**:
    - Showcase bukti kepuasan pembeli Determart, Essenz, Soft Sense, Malabeez, O Clean, Arai, Athari.
12. **Pilihan Paket Kemitraan**:
    - **Paket Starter**: Rp 15.000.000,-
    - **Paket King**: Rp 27.500.000,- (Harga Normal Rp 30.000.000,-)
13. **Cara Bergabung (3 Langkah Mudah)**:
    - Konsultasi Wilayah -> Pembayaran & Pengiriman Depot -> Training & Mulai Jualan.
14. **Footer 4-Kolom & Sticky Mobile Bar**:
    - Profil, Link Menu, 4 Program Ekosistem, Kontak Sleman, Hak Cipta.

---

## 3. Salinan Teks Lengkap (Wrapped Copywriting)

### Headline & Hero:
```text
Peluang Usaha Aneka Sabun dengan Potensi Omzet 50+ Juta/Bulan!
Jualan Sabun Curah Dengan Potensi Omzet 50+ Juta/Bulan! 
Produk gampang laku, cocok untuk pemula, minim resiko, pasti balik modal.
```

### Agitasi Masalah:
```text
Kamu Mau Sukses dalam Bisnis, Tapi...
- Pernah merasa bingung harus mulai dari mana?
- Tak tahu cara memasarkan produk atau layanan dengan efektif?
- Bingung dengan cara promosi yang tepat?
- Takut bisnis kamu gagal atau malah rugi?
- Takut ditipu atau justru gagal total?

Akibatnya:
Peluang besar terlewat begitu saja. Kamu terjebak dalam kehidupan finansial yang tidak berkembang. Mimpi besar malah terkubur dalam kondisi finansial yang terbatas. Dan itu membuat rasa menyesal semakin menguasai dirimu.
```

### Solusi Cleanique Mart:
```text
Cleanique Mart: Bisnis Aneka Sabun Auto Balik Modal!
Kami menawarkan produk homecare seperti deterjen, sabun, pelembut, dan pewangi yang selalu dibutuhkan oleh konsumen. Dengan inovasi sejak 2011, kami telah menjual lebih dari 1 juta produk, menjadi pilihan utama bagi banyak pelanggan.
- Tidak perlu repot cari produk
- Sistem sudah ada dan berjalan
- Produk sabun lengkap dan berkualitas dengan harga terjangkau
```

### Rincian Testimoni Mitra:
1. **Windu Darojat (Cleanique Mart Temanggung)**:
   > *"Sudah 19 tahun saya berbisnis pada sektor Household. Namun ketika saya menjadi Mitra Depo Cleanique banyak sekali dukungan yang tidak saya dapatkan di Perusahaan lain."*
2. **Johan Regan Jauw (Cleanique Mart Palembang)**:
   > *"Kami sangat mengapresiasi peluang bisnis ini dan berkomitmen untuk mengembangkan jaringan distribusi yang luas serta menyediakan produk chemical laundry dan rumah tangga berkualitas bagi masyarakat Kota Palembang."*
3. **Idha (Cleanique Mart Maguwoharjo Sleman)**:
   > *"Bimbingan Depo Cleanique sangat membantu saya dalam mengembangkan usaha isi ulang sabun. Omzet naik drastis, untung berlimpah!"*

### Rincian Paket Kemitraan:
#### 1. Paket Starter — Rp 15.000.000,-
- Rak Depot Display Eksklusif 8 Kontainer
- Pasokan Produk Siap Jual Senilai Rp 6.000.000,-
- Desain 3D Interior & Tata Letak Toko
- Lisensi Resmi Merk Cleanique Mart
- Peralatan Produksi Lengkap
- Include Biaya Pengiriman ke Lokasi Mitra
- Peralatan Packing & Segel
- Set-Up Toko di Marketplace (Shopee/Tokopedia)
- Media Promosi Cetak (Brosur, Banner, Spanduk)
- Instalasi Media Promosi Cetak di Lokasi Toko
- Training Chemical, SOP Pengisian & Manajemen Bisnis
- Landing Page Khusus SEO-Friendly
- Listing & Optimasi Titik Google Maps
- Set-Up dan Optimasi Media Sosial (Instagram/FB)
- Filter UV Higienis Air Isi Ulang

#### 2. Paket King — Rp 27.500.000,- (Harga Normal Rp 30.000.000,-)
- Seluruh 15 fasilitas yang ada di Paket Starter
- Pasokan Produk Siap Jual Lebih Besar Senilai Rp 8.000.000,-
- Pendampingan Bisnis Intensif & Hak Prioritas Wilayah

---

## 4. Implementasi Tema WordPress (`cleaniquemart-theme`)

- **File Template**: `front-page.php`
- **Template Parts yang Dipanggil**:
  ```php
  get_template_part('template-parts/hero-home');
  get_template_part('template-parts/metrics-bar');
  get_template_part('template-parts/problem-solution');
  get_template_part('template-parts/market-segments');
  get_template_part('template-parts/variants-grid');
  get_template_part('template-parts/testimonials-real');
  get_template_part('template-parts/reviews-lightbox');
  get_template_part('template-parts/packages-table');
  get_template_part('template-parts/steps-join');
  get_template_part('template-parts/cta-banner');
  ```
- **Customizer / ACF Fields yang Dibutuhkan**:
  - `home_hero_title`: Teks judul utama
  - `home_hero_badge`: Teks badge atas
  - `home_wa_phone`: Nomor WhatsApp CS (`087885590088`)
  - `home_proposal_url`: URL Google Drive Proposal

---

## 5. Integrasi Plugin (`cleaniquemart-core`)

- Shortcode `[cleanique_packages highlight="king"]` untuk menampilkan tabel paket Starter vs King.
- Shortcode `[cleanique_calculator]` jika ingin menyematkan simulator profit langsung di beranda.
- Event Trigger WA Modal: Tombol `"KONSULTASI GRATIS"` membuka modal registrasi dengan pesan default:
  ```
  Halo *cleaniquemart.com* Saya Mau Minta Informasi Tentang Mitra Cleanique Mart*
  ```
